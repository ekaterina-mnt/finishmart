    ...
    static function finalSubcategory($provider, $providerCategory = 'none', $providerSubcategory = null, $title = null, $product_link = null)
    {
        ...
        $keys = [
            ...
            7 => [ //'Комплектующие',
                ...
                ($provider == 'domix' and preg_match("#(Инсталляц|инсталляц|Каркас|Экран для ванн|Термостат|Базовый модуль|Пьедестал|Полупьедестал|Смывное устройство|Крышка-сидушка|Скрытая часть для|Излив|Уголок для|Слив|Крышка-сиденье|Комплект подключения|Клавиша)#", $title)),
            ...
            29 => [ //'Паркетная доска',
                ...
                ($provider == 'olimp' and preg_match("#(паркет|Паркет)#", $providerSubcategory) and !preg_match("#(для паркета)#", $providerSubcategory)),
            ],
            30 => [ //'Штучный паркет',
            ],
            31 => [ //'Виниловые полы',
