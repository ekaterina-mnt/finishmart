...
try {
    ...
    //тест
    $url = 'https://mosplitka.ru/catalog/svet/';
    ...
    foreach ($all_res as $href) {
        ...
        if (sql("SELECT id, link FROM test_links WHERE link='$link'")->num_rows) {
            continue;
        };
        ...
        if (isset($type)) {
            try {
                ...
                sql("INSERT INTO test_links (link, views, type, product_views) VALUES ('$link', 0, '$type', 0)");
                $add[] = $link;
            } catch (Exception $e) {
                continue; //для дублей
            }
