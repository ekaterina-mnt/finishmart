...
try {
    ...
    //Получаем ссылку
    $url_parser = $res['link'];
    $url_parser = "https://mosplitka.ru/catalog/plitka/view_product/?PAGEN_1=1";
    ...
    try {
        ...
        foreach ($all_res as $href) {
            ...
            //избавляемся от неподходящих ссылок
            $divided_link = array_slice(explode("/", $link), 4);
            if (preg_match("#.php$#", $link)) continue;
            if (!(in_array(count($divided_link), [1, 2]) or (in_array(count($divided_link), [1, 3]) and (strpos($link, "PAGEN"))))) continue;
