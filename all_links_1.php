...
try {
    for ($i = 1; $i < 6; $i++) {
        ...
        $query = MySQL::sql("SELECT link, views, provider FROM all_links WHERE type='catalog' and provider='centerkrasok' ORDER BY views, id LIMIT 1");
        ...
        foreach ($all_res as $href) {
            ...
            if ($provider == 'alpinefloor' and $href->attr('data-endpoint')) {
                $link = Parser::generateLink($href->attr('data-endpoint'), $provider, $url_parser);
            }
