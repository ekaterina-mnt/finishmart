...
$items = $document->find(implode(', ', $attributes_classes['items']));
...
if ($items) {
    foreach ($items as $num => $item) {
        ...
        //название товара
        $title_res = $item->find(implode(', ', $attributes_classes['title']));
        if ($title_res) {
        ...
        //цена
        $price_res = $item->find(implode(', ', $attributes_classes['price']));
        ...
        //наличие
        $stock_res = $item->find(implode(', ', $attributes_classes['stock']));
        if ($stock_res) {
        ...
        //артикул
        $articul_res = $item->find(implode(', ', $attributes_classes['articul']));
        if ($articul_res) {
        ...
        //категории из пути
        $path_res = $item->find(implode(', ', $attributes_classes['path']));
        if ($path_res) {
        ...
        //категория
        if ($provider == 'lkrn') {
            $categories_res = $item->find(implode(', ', $attributes_classes['category']));
        ...
        //все характеристики
        $characteristics_count = count($item->find(implode(', ', $attributes_classes['characteristics_count']))) - 1;
        $char_names = $item->find(implode(', ', $attributes_classes['char_name']));
        $char_values = $item->find(implode(', ', $attributes_classes['char_value']));
        $char_double_count = count($item->find(implode(', ', $attributes_classes['char_double_count']))) - 1;
        $char_double = $item->find(implode(', ', $attributes_classes['char_double']));
        ...
        if ($provider == 'lkrn') {
            ...
            foreach (range(0, count($char_names) - 1) as $charact) {
                ...
                if ((str_contains(mb_strtolower($name), "наименование") and $provider == 'artkera')) {
                    $all_product_data['title'] = [$value, 's'];
                }
        ...
        //картинки
        $images_res = $item->find(implode(', ', $attributes_classes['images']));
        if ($images_res) {
