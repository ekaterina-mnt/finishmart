...
try {
    for ($i = 1; $i < 2; $i++) { //тест
        ...
        $query = MySQL::sql("SELECT link, product_views, provider FROM all_links WHERE type='product' and provider='tdgalion' ORDER BY product_views, id LIMIT 1");
        ...
        $links = MySQL::sql("SELECT link, provider from all_products WHERE provider='tdgalion' and subcategory IS NULL or subcategory='null' ORDER BY date_edit LIMIT 10"); //тест
